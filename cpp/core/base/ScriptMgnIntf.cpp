...
class tTJSGlobalMissingHook : public tTJSDispatch {
    tjs_uint RefCount = 1;
public:
    tjs_uint AddRef() override { return ++RefCount; }
    tjs_uint Release() override { if(--RefCount == 0) { delete this; return 0; } return RefCount; }
    ...
    tjs_error FuncCall(tjs_uint32 flag, const tjs_char *membername, tjs_uint32 *hint, tTJSVariant *result, tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *objthis) override {
        if (numparams >= 3) {
            ...
            if (!is_set) {
                // Provide a Mock Object faking the property existence!
...
void TVPInitScriptEngine() {
    ...
    // Apply Universal Plugin/Class Mock Hook on Global
    iTJSDispatch2 *missHook = new tTJSGlobalMissingHook();
    tTJSVariant valMiss(missHook, missHook);
    missHook->Release();
    global->PropSet(TJS_MEMBERENSURE, TJS_W("missing"), nullptr, &valMiss, global);
    tTJSVariant missingName(TJS_W("missing"));
    global->ClassInstanceInfo(TJS_CII_SET_MISSING, 0, &missingName);
